...
func (m *Hero) ToORM(ctx context.Context) (HeroORM, error) {
	...
	// Repeated type LinkedAccount is not an ORMable message type
	to.Email = m.Email
	...
	to.Rating = m.Rating
	to.IsMentor = m.IsMentor
	to.IsApprentice = m.IsApprentice
	to.Xp = m.Xp
	to.Region = m.Region
	to.Language = m.Language
	to.Level = m.Level
	to.IsVerified = m.IsVerified
	to.TwoFactorAuth = m.TwoFactorAuth
	to.ReceiveVerificationText = m.ReceiveVerificationText
...
func DefaultApplyFieldMaskHero(ctx context.Context, patchee *Hero, patcher *Hero, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*Hero, error) {
	...
	var updatedMentor bool
	var updatedNotificationPreferences bool
	var updatedFinancials bool
	for i, f := range updateMask.Paths {
		...
		if f == prefix+"LinkedAccounts" {
			patchee.LinkedAccounts = patcher.LinkedAccounts
			continue
		}
		...
		if f == prefix+"IsMentor" {
			patchee.IsMentor = patcher.IsMentor
			continue
		}
		if !updatedMentor && strings.HasPrefix(f, prefix+"Mentor.") {
			if patcher.Mentor == nil {
				patchee.Mentor = nil
				continue
			}
			if patchee.Mentor == nil {
				patchee.Mentor = &Mentor{}
			}
			childMask := &field_mask.FieldMask{}
			for j := i; j < len(updateMask.Paths); j++ {
				if trimPath := strings.TrimPrefix(updateMask.Paths[j], prefix+"Mentor."); trimPath != updateMask.Paths[j] {
					childMask.Paths = append(childMask.Paths, trimPath)
				}
			}
			if err := gorm1.MergeWithMask(patcher.Mentor, patchee.Mentor, childMask); err != nil {
				return nil, nil
			}
		}
		if f == prefix+"Mentor" {
		...
		if f == prefix+"IsApprentice" {
			patchee.IsApprentice = patcher.IsApprentice
			continue
		}
		if f == prefix+"Xp" {
		...
		if f == prefix+"TwoFactorAuth" {
			patchee.TwoFactorAuth = patcher.TwoFactorAuth
			continue
		}
		if f == prefix+"ReceiveVerificationText" {
			patchee.ReceiveVerificationText = patcher.ReceiveVerificationText
			continue
		}
		if !updatedNotificationPreferences && strings.HasPrefix(f, prefix+"NotificationPreferences.") {
			if patcher.NotificationPreferences == nil {
				patchee.NotificationPreferences = nil
				continue
			}
			if patchee.NotificationPreferences == nil {
				patchee.NotificationPreferences = &NotificationPreferences{}
			}
			childMask := &field_mask.FieldMask{}
			for j := i; j < len(updateMask.Paths); j++ {
				if trimPath := strings.TrimPrefix(updateMask.Paths[j], prefix+"NotificationPreferences."); trimPath != updateMask.Paths[j] {
					childMask.Paths = append(childMask.Paths, trimPath)
				}
			}
			if err := gorm1.MergeWithMask(patcher.NotificationPreferences, patchee.NotificationPreferences, childMask); err != nil {
				return nil, nil
			}
		}
		if f == prefix+"NotificationPreferences" {
			updatedNotificationPreferences = true
			patchee.NotificationPreferences = patcher.NotificationPreferences
			continue
		}
		if !updatedFinancials && strings.HasPrefix(f, prefix+"Financials.") {
			if patcher.Financials == nil {
				patchee.Financials = nil
				continue
			}
			if patchee.Financials == nil {
				patchee.Financials = &Financials{}
			}
			childMask := &field_mask.FieldMask{}
			for j := i; j < len(updateMask.Paths); j++ {
				if trimPath := strings.TrimPrefix(updateMask.Paths[j], prefix+"Financials."); trimPath != updateMask.Paths[j] {
					childMask.Paths = append(childMask.Paths, trimPath)
				}
			}
			if err := gorm1.MergeWithMask(patcher.Financials, patchee.Financials, childMask); err != nil {
				return nil, nil
			}
		}
		if f == prefix+"Financials" {
			updatedFinancials = true
			patchee.Financials = patcher.Financials
			continue
		}
	}
	if err != nil {
